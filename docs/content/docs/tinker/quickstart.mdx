...
| `--database-url` | `sqlite:////tmp/skyrl_tinker/tinker.db` | Database URL for request tracking. If you point this at a SQLite file, keep it on node-local disk -- SQLite locking is unreliable on network filesystems (NFS) and causes `database is locked` errors under concurrency. |
