...
class TrainerConfig(BaseConfig):
    ...
    2, which would discard them; later sleeps stay at level 2. Rejected with ``dummy_initial_weights``,
    ``fp8_weight_sync_mode``, LoRA adapter sync, and the ``delta`` backend, which all rely on that
    first sync to supply real weights."""
    eval_interval: int = 5
