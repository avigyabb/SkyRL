...
async def served_app(tmp_path, monkeypatch):
    ...
    # Stores with a separate post-delivery clock (upstream) get it shortened so
    # the retry lands after it; a single completed-TTL store keeps its window.
    if hasattr(ExternalFutureStore, "_RETRIEVED_TTL_SECONDS"):
...
@pytest.mark.skipif(sys.platform != "linux", reason="relies on uvicorn disconnect handling over a real socket")
@pytest.mark.skipif(
    not hasattr(ExternalFutureStore, "_RETRIEVED_TTL_SECONDS"), reason="store has no post-delivery clock"
)
