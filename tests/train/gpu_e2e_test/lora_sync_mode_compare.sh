...
for MODE in $MODES; do
  echo "--- sync_mode=$MODE ---"
  # Trainer-side per-sync timing (both modes), the memory path's stage split, and the
  # rollout-vs-trainer logprob gap, which should match across modes.
  grep -E "timing/sync_weights|LoRA sync \(memory\)|LoRA sync: rank|rollout_train_logprobs_abs_diff_mean" "$LOG_DIR/$MODE.log" \
    | sed -E 's/^.*(timing\/sync_weights[^,}]*|LoRA sync[^"]*|rollout_train_logprobs_abs_diff_mean[^,}]*).*$/\1/' \
    | tail -n 16 || true
done
