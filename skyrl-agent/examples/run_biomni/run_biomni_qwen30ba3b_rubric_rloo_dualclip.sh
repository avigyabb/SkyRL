#!/usr/bin/env bash
# Biomni CodeAct 30B-A3B with RLOO + dual_clip (adapted from SA-SWE-32B recipe)
# Key changes from DRGRPO variant:
#   - advantage_estimator: rloo (leave-one-out, no std normalization)
#   - policy_loss_type: dual_clip (caps negative-advantage loss via clip_ratio_c)
#   - grpo_norm_by_std: false (no std normalization in advantage computation)
#   - LR scheduler: cosine_with_min_lr (decays to 1e-7 to prevent entropy collapse)
...
# ── Paths ──
PROJECT_NAME="biomni-training-qwen3-30b-a3b-skyrlagent-rubric-drgrpo"
EXPERIMENT_NAME="biomni-training-qwen3-30b-a3b-8gpus-rubric-rloo-dualclip-cosine"
...
# ── RL / optimization (RLOO + dual_clip, adapted from SA-SWE-32B recipe) ──
TRAIN_BATCH_SIZE=16
MINI_BATCH_SIZE=16
LR=1e-6
...
# ── Lengths ──
# Reduced from 40960 to leave headroom: standard PPO loss saves more
# intermediate tensors for backward than GSPO (which uses .detach())
MAX_PROMPT_LENGTH=38000
MAX_RESPONSE_LENGTH=4096
VLLM_MAX_MODEL_LEN=44000

# ── Agent config ──
AGENT_TASK_YAML="$(cd "$(dirname "$0")" && pwd)/../run_biomni/biomni_codeact_rubric_rl_qwen30ba3b_drgrpo.yaml"
