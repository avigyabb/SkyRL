#!/usr/bin/env bash
# DEBUG SCRIPT: 30B DRGRPO with minimal batch/trajectories for fast iteration
# Also includes the logprob diagnostic from megatron_model_wrapper.py
# Look for "[LOGPROB DIAG]" lines to check numerical discrepancy.
...
# ── DRGRPO clips (standard PPO range) ──
CLIP_RATIO_LOW=0.2
CLIP_RATIO_HIGH=0.28

# ── Shorter lengths for speed ──
MAX_PROMPT_LENGTH=16384
MAX_RESPONSE_LENGTH=4096
VLLM_MAX_MODEL_LEN=22000

# ── Debug YAML with num_trajectories=2 ──
AGENT_TASK_YAML="$(cd "$(dirname "$0")" && pwd)/../run_biomni/biomni_codeact_rubric_rl_qwen30ba3b_drgrpo_debug.yaml"
...
PYTHONUNBUFFERED=1 uv run --frozen --extra skyrl-train --env-file ~/SkyRL/skyrl-agent/examples/run_biomni/.env.biomni \
  ...
  trainer.ckpt_path="/tmp/debug_drgrpo_ckpt" \
  trainer.project_name="debug-30b-drgrpo" \
  trainer.run_name="debug-30b-drgrpo-clip0.28" \
