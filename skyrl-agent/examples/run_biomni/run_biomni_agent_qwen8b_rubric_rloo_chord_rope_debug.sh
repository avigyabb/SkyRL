#!/usr/bin/env bash
# DEBUG script for RLOO + dual_clip + CHORD correction auxiliary SFT loss (Qwen3-8B).
# Differences from production:
#   - BATCH_SIZE=4 (instead of 16)
#   - NUM_TRAJ=2 (instead of 5)
#   - Uses debug YAML with num_trajectories=2
#   - No checkpointing (ckpt_interval=999999)
#   - resume_mode=none (fresh start from SFT model)
#   - Console-only logger (no wandb)
...
PROJECT_NAME="debug-chord-correction"
EXPERIMENT_NAME="debug-chord-correction"
...
AGENT_TASK_YAML="$(cd "$(dirname "$0")" && pwd)/../run_biomni/biomni_codeact_rubric_rl_qwen8b_debug.yaml"
...
PYTHONUNBUFFERED=1 uv run --frozen --extra skyrl-train --env-file ~/SkyRL/skyrl-agent/examples/run_biomni/.env.biomni -m skyrl_agent.integrations.skyrl_train.skyrl_train_main \
  ...
  +trainer.correction_loss_mu=0.1 \
