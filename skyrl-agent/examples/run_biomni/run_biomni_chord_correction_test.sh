#!/usr/bin/env bash
# One-iteration test for full CHORD correction analysis.
# Generates 16 rollouts (8 prompts x 2 trajectories), evaluates rubric,
# generates corrections (format + rubric), and dumps all correction data
# to /tmp/chord_corrections_dump.jsonl for offline analysis.
#
# Usage: bash run_biomni_chord_correction_test.sh
...
# Enable verbose correction logging + JSONL dump
export CHORD_DEBUG_CORRECTIONS=1
export CHORD_DEBUG_DUMP_PATH="/tmp/chord_corrections_dump.jsonl"
rm -f "$CHORD_DEBUG_DUMP_PATH"
...
PROJECT_NAME="chord-correction-test"
EXPERIMENT_NAME="chord-correction-test"
...
AGENT_TASK_YAML="$(cd "$(dirname "$0")" && pwd)/../run_biomni/biomni_codeact_rubric_rl_qwen8b_chord_test.yaml"
