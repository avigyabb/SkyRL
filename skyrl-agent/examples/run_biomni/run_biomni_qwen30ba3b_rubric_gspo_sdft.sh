#!/usr/bin/env bash
# Biomni CodeAct training with GSPO + Self-Distillation Fine-Tuning (SDFT) for Qwen3-30B-A3B
# Adds SDFT auxiliary loss: EMA teacher conditioned on per-instance demonstrations,
# reverse KL(student||teacher) on student trajectories.
# SDFT config via Hydra CLI overrides at the bottom of this script.
...
# -----------------------------
PROJECT_NAME="biomni-training-qwen3-30b-a3b-skyrlagent-gspo-sdft"
EXPERIMENT_NAME="biomni-training-qwen3-30b-a3b-8gpus-rubric-gspo-sdft-reinforce"
...
INFER_TP=4
INFER_EP=1   # EP = DP * TP = 1 * 4 = 4 (must equal DP * TP)
INFER_DP=1
...
# TIS (for rollout-vs-train logprob mismatch, disabled for now until we fix the tok-in-tok-out inference issues)
USE_TIS=false
...
PYTHONUNBUFFERED=1 uv run --frozen --extra skyrl-train --env-file ~/SkyRL/skyrl-agent/examples/run_biomni/.env.biomni \
  ...
  generator.gpu_memory_utilization=0.2 \
