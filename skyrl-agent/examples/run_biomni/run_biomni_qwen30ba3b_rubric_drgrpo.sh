#!/usr/bin/env bash
# Biomni CodeAct 30B-A3B with DRGRPO (same recipe as the working 8B run)
# Key differences from GSPO variant:
#   - policy_loss_type: regular (default PPO, not GSPO)
#   - loss_reduction: seq_mean_token_sum_norm (Dr. GRPO, avoids length bias)
#   - eps_clip: 0.2/0.28 (standard PPO range, not GSPO's tight 3e-4/4e-4)
...
EXPERIMENT_NAME="biomni-training-qwen3-30b-a3b-8gpus-rubric-drgrpo-clip0.28"
...
# ── RL / optimization (DRGRPO recipe from 8B) ──
TRAIN_BATCH_SIZE=16
MINI_BATCH_SIZE=16
LR=1e-6
