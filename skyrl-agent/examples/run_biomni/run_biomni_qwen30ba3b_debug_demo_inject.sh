#!/usr/bin/env bash
# DEBUG: Biomni CodeAct 30B-A3B — test demonstration injection
# Runs rollout agent with demo injected into prompt (same as SDFT teacher would see).
# Compare rewards against baseline debug run (debug_gspo_dualclip_sdft_7.log) to
# validate that the demonstration improves gt, ft, and rubric rewards.
...
# ── Paths ──
PROJECT_NAME="debug-demo-inject"
EXPERIMENT_NAME="debug-demo-inject"
...
# ── RL / optimization (GSPO + dual_clip) — DEBUG: larger batch for less noise ──
TRAIN_BATCH_SIZE=4
MINI_BATCH_SIZE=4
LR=1e-6
...
# ── Agent config ──
AGENT_TASK_YAML="$(cd "$(dirname "$0")" && pwd)/../run_biomni/biomni_codeact_rubric_rl_qwen30ba3b_demo_inject_debug.yaml"
...
PYTHONUNBUFFERED=1 uv run --frozen --extra skyrl-train --env-file ~/SkyRL/skyrl-agent/examples/run_biomni/.env.biomni \
  ...
  trainer.ckpt_path="/tmp/debug_demo_inject_ckpt" \
