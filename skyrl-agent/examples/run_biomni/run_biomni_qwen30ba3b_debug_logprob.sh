#!/usr/bin/env bash
# DEBUG SCRIPT: Diagnose log-prob discrepancy between ref fwd and train fwd
# Runs 1 epoch with small batch to quickly check whether the Megatron
# reference and training forward passes produce identical log probs.
#
# Look for "[LOGPROB DIAG]" lines in the output.
# If frac>3e-4 is significantly non-zero at step 1, there's a numerical
# mismatch between the two forward passes.
...
# ── Parallelism (same as production) ──
NNODES=1
NUM_GPUS_PER_NODE=8
MEGATRON_TP=2
MEGATRON_PP=1
...
# ── GSPO with tight clips (what we're debugging) ──
EPS_LOW="3e-4"
EPS_HIGH="4e-4"

# ── Lengths (shorter to speed up rollouts) ──
...
PYTHONUNBUFFERED=1 uv run --frozen --extra skyrl-train --env-file ~/SkyRL/skyrl-agent/examples/run_biomni/.env.biomni \
  ...
  trainer.ckpt_path="/tmp/debug_logprob_ckpt" \
  trainer.project_name="debug-logprob-discrepancy" \
  trainer.run_name="debug-30b-gspo-logprob-check" \
