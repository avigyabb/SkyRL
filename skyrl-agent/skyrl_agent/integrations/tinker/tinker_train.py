...
class Config:
    ...
    # Uses a frozen teacher (base model) since Tinker has no EMA support.
    # The reinforce KL estimator is used (memory-free, logprob-only).
    # NOTE: When sdft_enabled=True, the effective gradient is averaged over 2N
    # examples (N policy + N SDFT) instead of N. This halves the effective LR
    # compared to the Megatron implementation. Compensate by doubling learning_rate.
    sdft_enabled: bool = False
    sdft_loss_coef: float = 1.0
...
def build_sdft_teacher_datums(
    sdft_data: List[Dict],
    response_ids: List[List[int]],
) -> tuple[List[types.Datum], List[int]]:
    """Build Tinker datums for teacher forward pass from SDFT data.

    The teacher input is: teacher_prompt + student_response.
    We use cross_entropy loss to get per-token logprobs without needing
    to provide advantages or sampling logprobs.

    Returns:
        teacher_datums: List of Datum objects for teacher forward_backward
        valid_indices: Indices into the original batch that have SDFT data
    """
    teacher_datums = []
    valid_indices = []

    for idx, entry in enumerate(sdft_data):
        if entry is None:
            continue

        teacher_prompt_ids = entry["teacher_prompt_ids"]
        resp_ids = entry["response_ids"]
        teacher_prompt_len = len(teacher_prompt_ids)

        full_sequence = teacher_prompt_ids + resp_ids
        target_tokens = full_sequence[1:]
        mask = [0.0] * teacher_prompt_len + [1.0] * len(resp_ids)
        mask = mask[1:]
        ...
        datum = types.Datum(
            ...
            loss_fn_inputs={
                ...
                "logprobs": TensorData.from_torch(torch.zeros(len(target_tokens))),
                "advantages": TensorData.from_torch(torch.zeros(len(target_tokens))),
            },
        )
        teacher_datums.append(datum)
        valid_indices.append(idx)

    return teacher_datums, valid_indices


def build_sdft_reinforce_datums(
    ...
    sdft_loss_coef: float,
) -> List[types.Datum]:
    """Build Tinker datums for the SDFT reinforce gradient step.

    Uses importance_sampling loss with:
      - advantages = sdft_coef * (student_logp - teacher_logp)  [the REINFORCE KL signal]
      - sampling_logprobs = student_logprobs  [so the IS ratio = 1]
      - mask = sdft_loss_mask from the SDFT data

    The resulting gradient is: sdft_coef * (log π_s - log π_t).detach() * ∇log π_s
    which is the REINFORCE estimator for ∇ KL(student ∥ teacher).
    """
    sdft_datums = []

    for i, orig_idx in enumerate(valid_indices):
        entry = sdft_data[orig_idx]
        prompt_ids = prompt_token_ids[orig_idx]
        resp_ids = response_ids[orig_idx]
        prompt_len = len(prompt_ids)
        teacher_prompt_len = len(entry["teacher_prompt_ids"])

        student_lp = student_logprobs_list[orig_idx]
        teacher_lp = teacher_logprobs_list[i]

        student_resp_lp = student_lp[prompt_len - 1:]
        teacher_resp_lp = teacher_lp[teacher_prompt_len - 1:]

        resp_len = len(resp_ids)
        min_len = min(len(student_resp_lp), len(teacher_resp_lp), resp_len)
        student_resp_lp = student_resp_lp[:min_len]
        teacher_resp_lp = teacher_resp_lp[:min_len]

        kl_signal = (student_resp_lp - teacher_resp_lp).detach()

        sdft_mask_raw = entry["loss_mask"][:min_len]
        sdft_mask = torch.tensor(sdft_mask_raw, dtype=torch.float32)

        full_sequence = prompt_ids + resp_ids
        target_tokens = full_sequence[1:]

        full_advantages = torch.zeros(len(target_tokens))
        full_sampling_lp = torch.zeros(len(target_tokens))
        full_mask = torch.zeros(len(target_tokens))

        start = prompt_len - 1
        end = start + min_len
        full_advantages[start:end] = sdft_loss_coef * kl_signal * sdft_mask
        full_sampling_lp[start:end] = student_resp_lp
        full_mask[start:end] = sdft_mask

        datum = types.Datum(
            model_input=types.ModelInput.from_ints(tokens=full_sequence[:-1]),
            loss_fn_inputs={
                "target_tokens": TensorData.from_torch(torch.tensor(target_tokens)),
                "logprobs": TensorData.from_torch(full_sampling_lp),
                "advantages": TensorData.from_torch(full_advantages),
            },
        )
        sdft_datums.append(datum)
...
async def main(config: Config):
    ...
    # Frozen teacher for SDFT: rank-1 LoRA adapter that is never updated.
    # At initialization the LoRA contribution is zero (B=0), so this is
    # equivalent to the base model. We flush its gradient accumulator each
    # step with lr=0 to bound memory.
    teacher_client = None
    if config.sdft_enabled:
        teacher_client = await service_client.create_lora_training_client_async(
            base_model=config.model_name, rank=1
        )
        logger.info("[SDFT] Created frozen teacher client (rank=1, never updated)")

    adam_params = types.AdamParams(learning_rate=config.learning_rate, beta1=0.9, beta2=0.95, eps=1e-8)
    teacher_flush_params = types.AdamParams(learning_rate=0.0, beta1=0.9, beta2=0.95, eps=1e-8)
    ...
    for policy_iteration_step in range(resume_from_step, config.max_steps):
        ...
        sdft_data = rollouts.get("sdft_data") if config.sdft_enabled else None

        if sdft_data and teacher_client is not None:
            # ---- SDFT-aware training: two forward_backward calls ----
            # 1) GSPO policy gradient (accumulates into student grad buffer)
            ...
            # 2) Teacher forward pass to get frozen-teacher logprobs
            teacher_datums, valid_indices = build_sdft_teacher_datums(sdft_data, response_ids)

            if teacher_datums:
                teacher_fwd_future = teacher_client.forward_backward(
                    teacher_datums, loss_fn="cross_entropy"
                )
                teacher_result = teacher_fwd_future.result()

                teacher_logprobs_list: list[torch.Tensor] = [
                    output["logprobs"].to_torch() for output in teacher_result.loss_fn_outputs
                ]

                # Flush teacher gradients (lr=0 keeps weights unchanged)
                teacher_client.optim_step(teacher_flush_params).result()

                # 3) Build SDFT reinforce datums and accumulate SDFT gradient
                sdft_datums = build_sdft_reinforce_datums(
                    prompt_token_ids, response_ids,
                    student_logprobs_list, teacher_logprobs_list,
                    sdft_data, valid_indices, config.sdft_loss_coef,
                )

                if sdft_datums:
                    sdft_fwd_future = training_client.forward_backward(
                        sdft_datums, loss_fn="importance_sampling"
                    )
                    sdft_fwd_result = sdft_fwd_future.result()

                    sdft_kl_values = []
                    for i, orig_idx in enumerate(valid_indices):
                        entry = sdft_data[orig_idx]
                        s_lp = student_logprobs_list[orig_idx]
                        t_lp = teacher_logprobs_list[i]
                        p_len = len(prompt_token_ids[orig_idx])
                        tp_len = len(entry["teacher_prompt_ids"])
                        s_resp = s_lp[p_len - 1:]
                        t_resp = t_lp[tp_len - 1:]
                        ml = min(len(s_resp), len(t_resp))
                        if ml > 0:
                            sdft_kl_values.append((s_resp[:ml] - t_resp[:ml]).mean().item())

                    if sdft_kl_values:
                        metrics["sdft/kl_mean"] = np.mean(sdft_kl_values)
                        metrics["sdft/num_valid"] = len(sdft_kl_values)

                    logger.info(
                        f"[SDFT] Processed {len(sdft_datums)} teacher-student pairs, "
                        f"KL mean={metrics.get('sdft/kl_mean', 'N/A')}"
                    )

            # 4) Apply combined GSPO + SDFT gradient update
            optim_step_future = training_client.optim_step(adam_params)
            _ = optim_step_future.result()
        else:
            # ---- Standard training (no SDFT) ----
            fwd_bwd_future = training_client.forward_backward(training_datums, loss_fn=config.loss_fn)
            optim_step_future = training_client.optim_step(adam_params)
            fwd_bwd_result = fwd_bwd_future.result()
            _ = optim_step_future.result()
